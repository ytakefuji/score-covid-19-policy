...
#
 ...
 dd=pd.DataFrame(
  { 
   ...
   "score": range(len(d)),
   "VSL": range(len(d)),
   "covidloss": range(len(d)),
  })
 
 vsl=pd.read_csv('VSL_data.csv',encoding='UTF-8')
 ...
 # print(p[i][len(p)-1])
  ...
  #print(vsl.loc[vsl.country==i,'VSL'])
  ...
  dd.loc[dd.country==i,'covidloss']=round(dd.loc[dd.country==i,'deaths']*float(vsl.loc[vsl.country==i,'VSL']),1)
 dd=dd.sort_values(by=['covidloss'])
